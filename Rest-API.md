# REST API Görev Dağılımı

**REST API Adresi:** [RateFlix backend adresi](https://rateflix-backend.onrender.com)

Bu dokümanda, proje ekibindeki her üyenin geliştirmekten sorumlu olduğu REST API metotları listelenmektedir.

---

## Grup Üyelerinin REST API Metotları

1. [Mahmut Kesen'in REST API Metotları](Mahmut-Kesen/Mahmut-Kesen-Rest-API-Gorevleri.md)
2. [Aytuğ Avni Akay'ın REST API Metotları](Aytuğ-Akay/Aytuğ-Akay-Rest-API-Gorevleri.md)
